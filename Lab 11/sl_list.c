...
intnode_t *add(intnode_t *head, int elem, int index)
{
    ...
    if(head == NULL){
        assert(index == 0);
        head = malloc(sizeof(intnode_t));
        assert(head != NULL);

        *head = (intnode_t) { elem, NULL };
...
int count_in_sll(intnode_t *head, int target)
{
    int count = 0;
    if(head != NULL){
        count = count_in_sll(head->next, target);
